# 03_rf_train_validate.R
# Random Forest (ranger) top-K training + external validation (fixed threshold = 0.6)

packages <- c(
  "tidyverse", "tidymodels", "ranger", "readxl",
...
# ================== Tune per-K ==================
tune_rf_for_k <- function(K, dat, rf_rank) {
...
fit_final_rf <- function(dat, sel_vars, rec, best_params) {
...
all_results <- lapply(KS_TARGET, function(K) tune_rf_for_k(K, dat, rf_rank))
...
openxlsx::write.xlsx(summary_tbl, "rf_cv_auc_summary.xlsx", asTable = TRUE)

train_eval <- list()

for (res in all_results) {
  K <- res$K
  fit_final <- fit_final_rf(dat, res$sel_vars, res$recipe, res$best_params)
  ...
  saveRDS(fit_final, file = paste0("rf_model_K", K, ".rds"))
}

train_eval_tbl <- bind_rows(train_eval)
openxlsx::write.xlsx(train_eval_tbl, "rf_train_eval_threshold_0p6.xlsx", asTable = TRUE)
...
for (nm in names(EXT_SETS)) {
  ...
  for (res in all_results) {
    K <- res$K
    fit_final <- readRDS(paste0("rf_model_K", K, ".rds"))
    ...
    openxlsx::write.xlsx(
      pred_export,
      paste0("rf_", nm, "_K", K, "_predictions_threshold_0p6.xlsx"),
      asTable = TRUE
    )
  }
}
...
openxlsx::write.xlsx(ext_eval_tbl, "rf_external_eval_threshold_0p6.xlsx", asTable = TRUE)
