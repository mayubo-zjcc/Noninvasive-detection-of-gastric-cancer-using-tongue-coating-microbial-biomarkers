# 01_xgb_train_validate.R
# XGBoost top-K training + external validation (fixed threshold = 0.6)

packages <- c(
  "tidyverse", "tidymodels", "xgboost", "readxl",
  "stringi", "openxlsx", "rlang"
)
...
# External datasets (edit as needed)
EXT_SETS <- list(
...
metrics_at_threshold <- function(df_prob, truth_col, prob_col, threshold, pos, neg) {
  ...
  cm <- yardstick::conf_mat(df_cls, truth = !!sym(truth_col), estimate = .pred_class)
...
# ================== Tune per-K ==================
tune_xgb_for_k <- function(K, dat, rf_rank, POS_CLASS) {
...
fit_final_xgb <- function(dat, sel_vars, rec, best_params) {
...
all_results <- lapply(KS_TARGET, function(K) tune_xgb_for_k(K, dat, rf_rank, POS_CLASS))
...
openxlsx::write.xlsx(summary_tbl, "xgb_cv_auc_summary.xlsx", asTable = TRUE)

# Fit final models and evaluate on training set with fixed threshold
train_eval <- list()

for (res in all_results) {
  K <- res$K
  fit_final <- fit_final_xgb(dat, res$sel_vars, res$recipe, res$best_params)
  ...
  saveRDS(fit_final, file = paste0("xgb_model_K", K, ".rds"))
}

train_eval_tbl <- bind_rows(train_eval)
openxlsx::write.xlsx(train_eval_tbl, "xgb_train_eval_threshold_0p6.xlsx", asTable = TRUE)
...
for (nm in names(EXT_SETS)) {
  ...
  for (res in all_results) {
    K <- res$K
    fit_final <- readRDS(paste0("xgb_model_K", K, ".rds"))
    ...
    openxlsx::write.xlsx(
      pred_export,
      paste0("xgb_", nm, "_K", K, "_predictions_threshold_0p6.xlsx"),
      asTable = TRUE
    )
  }
}
...
openxlsx::write.xlsx(ext_eval_tbl, "xgb_external_eval_threshold_0p6.xlsx", asTable = TRUE)
